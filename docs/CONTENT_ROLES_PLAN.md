...
360×640·390×844 두 해상도에서 카드·랭킹·성장 선택을 다시 재고 세 가지를 더 고쳤다.

- **낮은 화면의 방치 정보**: 블록 카드를 숨기기만 하면 360×640 유저는 "닫아도 쌓인다"를 못 본다. 필드 HP 바 바로 아래에 한 줄 스트립(● 자동 사냥 · 골드/EXP/강화석 아이콘 + 시간당 · 최대 Th)을 띄워 레이아웃을 밀지 않고 보여 준다(inspect-mobile 0 겹침). 720px 초과에서는 블록 카드, 이하에서는 스트립.
- **한글 줄바꿈**: 성장 선택 안내문·방치 카드 문장·배너 힌트가 "적/용되는", "정/산됩니다"처럼 단어 중간에서 꺾이던 것을 `word-break: keep-all` 로 고정.
- **스트립 상태 점**: flex 안에서 1px 선으로 찌그러지던 것을 고정 폭으로.
- 랭킹 행(폭 310px)·성장 선택 버튼(320×76, 0/90/180ms 팝인)·스위프 애니메이션은 두 해상도 모두 측정값으로 확인.
