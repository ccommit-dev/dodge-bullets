...
import { dailyMissionsDone, dateKey, RIFT_ATTEMPTS, type EventSave } from "../events/eventSave";
...
export function routineItems(progress: CharacterProgress, events: EventSave, now = Date.now()): RoutineItem[] {
  ...
  const riftLeft = Math.max(0, RIFT_ATTEMPTS - events.riftAttempts);
  ...
  return [
    ...
    { id: "rift", label: "균열", detail: riftLeft === 0 ? "3/3 완료" : `${RIFT_ATTEMPTS - riftLeft}/${RIFT_ATTEMPTS}`, done: riftLeft === 0, go: { kind: "events", tab: "rift" } },
