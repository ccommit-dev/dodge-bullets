...
export function TitansGame({ insets, userHash, forgedWeaponLevel = 0, onOpenContent }: TitansGameProps) {
  ...
  const castSkill = (id: TitanSkillId) => {
    ...
    if (def.slot === "starter") {
      ...
      applyDamage(dmg * (id === "pierce" ? 52 : id === "emberCut" ? 46 : id === "frostEdge" ? 43 : 40), true);
      flash(`${def.name}!`);
      return;
    }
  ...
  return (
    <div className="titans-layer" style={pad}>
      ...
      <div className="titans-skills-row">
        ...
        {SKILLS.filter((sk) => save.skillInventory.equipped[sk.slot] === sk.id).map((sk) => {
      ...
      <section className="titans-shop">
        ...
        {tab === "premium" && premiumCategory === "package" && STORE_PRODUCTS.filter((product) => product.visible).map((product) => {
