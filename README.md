...
2. **비트박스 Orbit** — [Orbit or Beat](https://vidkidz.tistory.com/84) 원버튼 반전  
   - STAGE 1→6 캠페인 · 클리어하면 오버레이 없이 다음 스테이지로 이어짐
   - 탭/스페이스 = 궤도 반전 + 비트박스 리드
   - 저음 클럽 킥·베이스 + 클릭이 곡을 완성
   - 단일 3D 궤도 · 클러치 판정 · 파티클
   - **밝게 빛나는 호 = 지금 달려가는 방향**(어두운 쪽은 지나온 길), 셰브론이 진행 방향 표시
   - 가시는 항상 진행 방향 앞쪽에만, 뒤쪽 탈출로가 비어 있을 때만 생성 → 반전이 언제나 유효한 탈출
