/**
 * 화면 촬영 — 허브·동료 시트·뽑기 시트·상점 시트·대장간 검/견갑·비트 메뉴. UI 배치 점검용.
...
const openContent = async (label) => { await clickText(".titans-bottom-nav button", "콘텐츠"); await sleep(400); await clickText(".nav-popup-grid button", label); await sleep(1400); };
...
console.log("hub boxes", JSON.stringify({ hp: await box(".titans-hp"), alerts: await box(".battle-alert-stack"), alert0: await box(".battle-alert"), nav: await box(".titans-bottom-nav"), field: await box(".titans-field"), dock: await box(".titans-skill-dock") }));
console.log("boss cta css", JSON.stringify(await page.evaluate(() => { const el = document.createElement("button"); el.className = "titans-boss-challenge"; el.textContent = "보스 도전하기"; document.querySelector(".titans-field").appendChild(el); const r = el.getBoundingClientRect(); const f = document.querySelector(".titans-field").getBoundingClientRect(); const hp = document.querySelector(".titans-hp").getBoundingClientRect(); const out = { centerDx: Math.round(r.x + r.width / 2 - (f.x + f.width / 2)), gapBelowHp: Math.round(r.y - hp.bottom) }; el.remove(); return out; })));
